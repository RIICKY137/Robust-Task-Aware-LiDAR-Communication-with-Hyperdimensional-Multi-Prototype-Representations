#!/usr/bin/env python3
"""Few-shot OOD remake at the 128 B operating point.

k=1 / k=16 centroids vs a linear head. Shots are balanced per class on
`test_ood`; old-task accuracy is `test_id`. Does not overwrite
`multicentroid_adaptation.jsonl` or `adaptation.jsonl`.
"""
...
def main() -> None:
    ...
    out = ROOT / "results" / "raw" / "k16_adaptation_128b.jsonl"
    ...
    for seed in SEEDS:
        ...
        for tag, kw in METHODS:
            ...
            method = build_method("pure_hdc", BUDGET, seed=seed, dimension=DIMENSION, **kw)
            method.fit(train.ranges, train.labels, train.max_range)
            assert method.dimension == DIMENSION
